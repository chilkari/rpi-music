...
config(
    ...
    in_ports=['nanoKontrol_IN', 'midiport'],
    out_ports=['setBfree','nanoKontrol_OUT'],
...
Filter(NOTE) >> [
        Port('setBfree'), 
        Port('nanoKontrol_OUT') >> Channel(14) >> Key(35)
    ]
]
organ_lower = Output('setBfree', 2)
organ_pedal = Output('setBfree', 3)
